...
{   
    if(card->dsc_buffs[class_index]->head == 0)
        card->dsc_buffs[class_index]->tail = ((card->dsc_buffs[class_index]->mask)>>6);
    else
        card->dsc_buffs[class_index]->tail = card->dsc_buffs[class_index]->head - 1;
    
    printk(KERN_EMERG "begin adding doorbell\n");
    doorbell_add_dsc(card, class_index, card->dsc_buffs[class_index]->pkt_physical_addr, 0x8ULL,
